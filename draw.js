let on = false, saveR = [], st, tt = false,
...
function erase2(e){
    ...
    let final = (e) => {
    if(fin){
        rayc.push(new cleaned(span.offsetLeft, span.offsetTop, span.offsetWidth, span.offsetHeight));
        line.clearRect(span.offsetLeft, span.offsetTop, span.offsetWidth, span.offsetHeight);
    }
    }
    ...
    ccl = (a) => {
        ...
        line.clearRect(ray[a].logX[i] - ((ray[a].l - 1)/ 1.8), ray[a].logY[i] - ((ray[a].l - 1)/ 1.8) - 1, ray[a].l + 2, ray[a].l);
    }},
    set = (x) => {
         line.lineCap = "round";
         line.lineJoin = "round";
